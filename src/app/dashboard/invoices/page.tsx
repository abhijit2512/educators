        ...
        {invoices.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-left text-xs uppercase text-slate-500">
              <tr><th className="px-4 py-2">Number</th><th className="px-4 py-2">Amount</th><th className="px-4 py-2">Status</th><th className="px-4 py-2">Issued</th><th className="px-4 py-2"></th></tr>
            </thead>
            <tbody>
              {invoices.map((i) => (
                <tr key={i.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-2"><Link className="text-brand-700 hover:underline" href={`/dashboard/invoices/${i.id}`}>View</Link></td>
                </tr>
              ))}
            </tbody>
          </table>
